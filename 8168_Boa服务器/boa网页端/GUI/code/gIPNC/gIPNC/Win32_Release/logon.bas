...
Checks Ä whether remember password is enabled by reading the Name Value pair from temperory memory(readprofile) 
Checks which page Ä load based on the event. F5/refresh - load live video ÄC login page
*/
 ...
 * F5/refresh - load live video ÄC login page
...
Äh Form_Load  
 ...
 ÄB splitcount = 3 Ä 
 #imgLoading.x = Äø()/2 - 200
 #imgLoading.y = Ä¿()/2 - 100
 
 ~authUserName$ = Ä#(tempDet$(0)) 
 ~authPassword$ = Ä#(tempDet$(1)) 
