 ...
 * Description: 
 * Ä set the X,Y positions Ä all the Controls with respect Ä the Resolution
...
Äh form_load
 ...
 Ä disableScheduleDrp() 
 
 Ä loadDropDownValues() 
 
 ÄB #optrepeatschedule = 1 Ä
 #txtweeks.disabled = 0
 ÄC 
 #txtweeks.disabled = 1
 ÄD 
 ...
 * Description: 
 * set disable color Ä the option buttons
 ...
 * Description: Ä this Ä Ä disable drop downs ÄB check  
 * box is unchecked.

 * Created by:S.Vimala On 2009-08-31 17:29:36
 ***********************************************************/
Äh disableScheduleDrp() 
 dimi i,j,xval,yval
 ...
 * Description: Ä this Ä Ä load schedule drop down values
 
 * Created by: S.Vimala On 2009-08-31 17:53:16
 ***********************************************************/
Äh loadDropDownValues() 
 dimi loopcnt,i
 dims ctrlName$  
 Ä i = 1 Ä 7
 ctrlName$ = "drpday"+i
 Ä loopcnt = 0 Ä ÄÇ(day$) 
 ...
 * Description: Ä1 scheduler.
 Gray controls based on the control disable property
 
 * Created by: Franklin Jacques On 2009-03-12 17:21:45
 ***********************************************************/
Äh form_paint
 Ä #frSchedule.hidden=1 and paintFlag=1 Ä Ä drawScheduler() 
 
 ÄB #ChkUploadViaFtp.disabled = 1 Ä 
 ...
 ÄB #chkLocalStorage.disabled = 1 Ä 
 ...
 Ä #frschedule.hidden = 0 Ä  Ä grayDropdown() 
 ...
 * Description: Ä this Ä Ä gray the drop down boxes ÄB unchecked
 * 
 ...
 * Description: 
 * Ä& the X and Y pos Ä draw the scheduler window.
 ...
 * Description: 
 * Ä set the value Ä the Check box ctrl-chkUploadViaFtp,
 chkLocalStorage 
 Store all the control values in an array Ä validate changes in form.

 * Created by: On 2009-05-19 15:29:32
 ***********************************************************/
Äh form_complete
 Ä chkUploadViaFtp_Click
 ...
 * Description: Ä set the initial values Ä the controls and schedule Ä 
 ...
 * History: 
 ***********************************************************/
Äh setInitialScheduler() 
 dimi row, col
 dims ddValue$
 
 
 Ä row = 0 Ä MAX_DAYS-1
 Ä col=0 Ä totSchedule-1
 ...
 ÄB ret = 0 Ä
 Ä setValuesToDropDown(recordSchedule$)
 #rocamera$ = ~title$
 Ä<(ddValue$,ftpfileformatname$,";")
 Ä addItemsToDropDown("ddstorageformat", ddValue$, ftpfileformat)
 Ä<(ddValue$,sdfileformatname$,";")
 Ä addItemsToDropDown("ddstorageformat1", ddValue$, sdfileformat)
 Ä uploadbyFTP = 0 or uploadbyFTP = 1 Ä #ChkUploadViaFtp$=uploadbyFTP
 Ä storeLocally = 0 or storeLocally = 1 Ä #ChkLocalStorage$=storeLocally
 Ä localStorage = 0 or localStorage = 1 or localStorage = 2 Ä #optlocalstorage$=localStorage
 Ä repeatSchedule = 0 or repeatSchedule = 1 Ä #optRepeatSchedule$=repeatSchedule
 Ä scheduleInfinity = 0 or scheduleInfinity = 1 Ä #optruntimeinfinite$=scheduleInfinity
 #txtweeks$=noOfWeeks

 
 ÄB #ddstorageformat.itemcount>0  Ä
 ÄB #ddstorageformat.itemlabel$(#ddstorageformat.selidx)="N/A" Ä 
 ...
 ÄB #ddstorageformat1.itemcount>0 Ä
 ÄB  #ddstorageformat1.itemlabel$(#ddstorageformat1.selidx)="N/A" or sdCard = 0 or checkSDInsertValue()= 1 Ä 
 ...
 * Description: 
 * Ä set the schedule details Ä the dropdown boxes of schedule frame
 * 
 * Params:
 * dims recordSchedule$: String - detail of the selected schedule
 * Created by: On 2009-09-01 17:04:10
 ***********************************************************/
 Äh setValuesToDropDown(dims recordSchedule$()) 
 ...
 i=i+1
 ÄB TempString$(2)=1 or TempString$(2)=0 Ä
 ctrlName$ = "chkschedule"+i
 ...
 ÄB strtoint(#{ctrlName$}$)="00" and strtoint(#{ctrlName1$}$)="00" and strtoint(#{ctrlName2$}$)="23" and strtoint(#{ctrlName3$}$)="59" and strtoint(TempString$(15))=5 and strtoint(TempString$(16))=9 Ä
 #{ctrlName2$}$="00"
 #{ctrlName3$}$="00"
 ÄD
 ÄD
 ...
 * Description: Ä show and hide save success message  
 ...
 * Description: Ä the save the changed values Ä the
 controls and the scheduler
 *
 * 
 * Methods: SavePage- Ä the save the changed values Ä the
 controls and the scheduler
 ...
 * Description: 
 * Validate  and Save schedule values Ä camera
...
Äh savePage()
 ...
 dimi noOfWeeks
 noOfWeeks = strToint(Ä#(#txtweeks)) 
 ÄB (noOfWeeks<1 or noOfWeeks>52) and #optrepeatschedule=1 Ä
 ~mousemoveflag=1
 Ä("Number of weeks should be between 1 to 52")
 Äd("txtweeks")
 ~mousemoveflag=0
 Ä
 ÄD
...
Äh displaySaveStatus(dimi saveStatus)
 ...
 ~mousemoveflag=1
 ÄB ~keywordDetFlag = 1 Ä
 Ä("Storage setting for \n"+~errorKeywords$+"\nfailed for the camera "+~title$)
 ÄC 
 Ä("Storage setting failed for the camera "+~title$)
 ÄD
 ~mousemoveflag=0
 Ä setInitialScheduler()
 ÄD
 ...
 * Description: 
 * Ä clear the values of the dropdown boxes and set the 
 default values Ä it

 * Created by: Vimala On 2009-09-02 16:12:55
 ***********************************************************/
Äh ClearDropDownValues()
 dims ctrlName1$
 dims ctrlName2$
 dims ctrlName3$
 dims ctrlName4$
 ...
 * History: 
 ***********************************************************/
Äh optionRunTimeInfinite_Click
 ...
 * Description: Ä load the default values set initially Ä 
...
Äh drawScheduler()
 ...
 Ä\(xpos, scheduleY, SCHEDULE_W, SCH_TIT_H, SCH_TIT_BG)
 
 
 txtY = 24 - Ä«("Schedule", SCH_TIT_STYLE, SCH_TIT_FONT)
 Ä^(xPos+SCH_TIT_X_GAP, scheduleY+txtY-2, "Schedule", SCH_TIT_STYLE, SCH_TIT_FG, SCH_TIT_FONT)
 ...
 Ä i=0 Ä maxDur
 timeVal = i*6
 Ä^((xPos+DAY_DISPLAY_W-DAY_X_GAP)-DAY_X_GAP, yPos, Ä$("2.2", timeVal), HOUR_TXT_STYLE, HOUR_TXT_FG, HOUR_TXT_FONT)
 xPos += durHourW
 Ä
 ...
 Ä^(xPos+DAY_X_GAP, yPos-DAY_HEIGHT, day$(i), DAY_TXT_STYLE, DAY_TXT_FG, DAY_TXT_FONT, DAY_DISPLAY_W-DAY_X_GAP)
 
 xPos += 150

 Ä j=0 Ä totSchedule-1
 
 ÄB j%(maxDur) = 0 and j <> 0 Ä
 ÄZ(xPos+1, lineYPos, xPos+1, lineYPos+SCH_IDX_LINE_H, 1, SCH_IDX_BG)
 ÄD
 
 Ä j <> 0 Ä xPos += 3
 
 ÄB scheduleIdx(i,j) = 0 Ä
 
 Ä\(xPos, yPos, SCH_IDX_W, SCH_IDX_H, SCH_IDX_BG)
 Äy scheduleIdx(i,j) = 1 Ä
 SelectedSchedule(xPos, yPos, CHK_BLUE, 1, j) 
 Äy scheduleIdx(i,j) = 2 Ä  
 SelectedSchedule(xPos, yPos, CHK_GREEN, 2, j) 
 Äy scheduleIdx(i,j) = 3 Ä  
 SelectedSchedule(xPos, yPos, CHK_PURPLE, 3, j) 
 Äy scheduleIdx(i,j) = 4 Ä
 SelectedSchedule(xPos, yPos, CHK_RED, 4, j) 
 Äy scheduleIdx(i,j) = 5 Ä
 SelectedSchedule(xPos, yPos, CHK_AQUA, 5, j)
 Äy scheduleIdx(i,j) = 6 Ä
 SelectedSchedule(xPos, yPos, CHK_THICKGREEN, 6, j)
 Äy scheduleIdx(i,j) = 7 Ä
 SelectedSchedule(xPos, yPos, CHK_ORANGE, 7, j)
 Äy scheduleIdx(i,j) = -1 Ä
 Ä\(xPos, yPos, SCH_IDX_W, SCH_IDX_H, DISABLE_COLOR,,,ALPHA_LEVEL) 
 ÄD
 ...
 * Description: 
 * Ä set the color, x position, Y position and Alpha level
 Ä highlight the selected schedule with the subsequent color
 ...
 * History: 
 ***********************************************************/
Äh SelectedSchedule(dimi xPos, dimi yPos, dimi curColor, dimi index, dimi j)
 index=index-1
 dimi frmTime
 dimi toTime
 dimi  selFlag=0
 ...
 Ä\(xPos, yPos, SCH_IDX_W, SCH_IDX_H, curColor,,,ALPHA_LEVEL)
ÄF
 ...
 ***********************************************************/
Äh fillScheduleData(dimi startIdx, dimi endIdx, dimi srcIdx)
 
 dimi fillData, i
 dimi scheduleCount,j
 
 ÄB scheduleIdx(selIdxRow, srcIdx) = 0 Ä  
 fillData = 1
 ÄC
 fillData = 0  
 ÄD
 
 Ä i = startIdx Ä endIdx
 ...
 Ä j=0 Ä totSchedule-1
 ÄB scheduleIdx(i,j) = 1 Ä
 
 scheduleCount++
 
 ÄB scheduleCount > 7 Ä  
 fillData = 0
 
 
 ÄB endIdx < totSchedule-1 Ä
 Ä  scheduleIdx(selIdxRow, endIdx+1) = 1 Ä fillData = 1
 ÄD
 ...
 selIdxRow = -1
 Ä("You have already created maximum number of schedules")
 Ä
 ÄD
 
 Äv(scheduleIdx(i,j) = 1) 
 j++
 Ä j >= totSchedule Ä ÄE
 Äw
 ...
 * Description: procedure Ä display the tool tip(selected schedule's From & Ä Ä) 
 during the mouse-over on the selected schedule.

 * Created by: Jacques Franklin On 2009-04-25 15:16:04
 ***********************************************************/
Äh ToolTip_MouseOver(dimi x,dimi y)
 Dims txtcnt$,st$,st1$,stpopup$,endtPopup$,tempString$
 dimi endIdx,i,j,ypos,selectedrow1,schDFlag, curId 
 schDFlag=0
 curId=-1
 ÄB mouseClickflag=1 Ä
 Ä i=0 Ä 6 
 ÄB x >= scheduleX+150  and x <= (scheduleX+150 +((SCHEDULE_W)-DAY_DISPLAY_W)) and y >= scheduleY+((i+2)*SCH_IDX_H)+((i+2)*SCH_IDX_Y_GAP) and y <= scheduleY+SCH_IDX_H+((i+2)*10*~factorY)+((i+2)*20*~factorY) Ä
 schDFlag=1
 ÄE
 ÄD
 Ä  
 ...
 ÄB schDFlag=1 Ä
 schDFlag=0
 Ä x < scheduleX+150  Ä x = scheduleX+150 
 Ä1(1)
 endIdx = Äƒ(x-(scheduleX+DAY_DISPLAY_W))/6  
 yPos = scheduleY + 24 + 2*20 * ~factorY + 2
 
 Ä i=0 Ä MAX_DAYS-1
 
 ÄB y >= yPos and y <= yPos+10 * ~factorY and x >= scheduleX+150  and x <= scheduleX+150 +(SCHEDULE_W-DAY_DISPLAY_W) Ä  
 selectedrow1 = i  
 ÄE
 ÄC
 selectedrow1=-1 
 ...
 Ä
 ÄB endidx<=95 and selectedrow1<>-1 Ä
 curId = scheduleIdx(selectedrow1,endIdx)
 curId=curId-1
 ÄD
 Ä curId<0 Ä Ä
 ÄB selectedrow1<>-1 and endIdx<=95 Ä
 ÄB scheduleIdx(selectedrow1,endIdx)<>0 and endIdx<=95 Ä
 ÄB endIdx>0 Ä
 Ä i=endIdx Ä 0 Ä2 -1
 ÄB scheduleIdx(selectedrow1,i)= 0 Ä
 ÄE
 ÄD
 Ä
 ÄD
 ÄB endIdx<95 Ä
 Ä j=endIdx Ä 95
 ÄB scheduleIdx(selectedrow1,j) = 0 Ä
 ÄE
 ÄD
 Ä
 ÄD
 ...
 Ä1(prvx,prvy,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT)
 ÄB endIdx>=80 Ä
 Ä—(prvScreen$,x-(TOOLTIP_WIDTH)/2,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT)
 ÄC
 Ä—(prvScreen$,x+10,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT)
 ÄD
 Ä“(prvScreen$,prvX,prvY,5,1)
 ÄB curId>=0 Ä
 //tool tip-Ä (From:Ä) 
 txtcnt$ = frmHrs$(curId)+":"+frmMin$(curId)+" : "+toHrs$(curId)+":"+toMin$(curId)
 ÄD
 ÄB endIdx>=80 Ä
 Ä[(x-(TOOLTIP_WIDTH)/2,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT,1,1,DISPLAY_REGION_BG,DISPLAY_REGION_BG)
 Ä^(x-(TOOLTIP_WIDTH)/2,y,txtcnt$,DAY_TXT_STYLE, TOOLTIP_COLOR, DAY_TXT_FONT)
 Ä1(x-(TOOLTIP_WIDTH)/2,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT)
 ÄC
 Ä[(x+10,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT,1,1,DISPLAY_REGION_BG,DISPLAY_REGION_BG)
 Ä^(x+10,y,txtcnt$,DAY_TXT_STYLE, TOOLTIP_COLOR, DAY_TXT_FONT)
 Ä1(x+10,y,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT)
 ÄD
 ÄB endIdx>=80 Ä
 ...
 prvY=-100
 Ä form_paint
 Ä1(prvX,prvY,(TOOLTIP_WIDTH)/2,TOOLTIP_HEIGHT+1)
 ÄD
 ÄD
 ÄC
 prvX=-100
 ...
 * Description: 
 * Select the schduler Ä selected/unselected.
 * 
 * Params:
...
Äh Form_MouseMove( x, y )
 ÄB ~mousemoveflag=0 Ä
 Ä #frSchedule.hidden=1 and paintFlag=1 Ä Ä ToolTip_MouseOver(x,y) 
 ÄD 
 Ä0(0) 
 ...
 * History: 
 ***********************************************************/
/*Äh displayCurSchedule(dimi startIdx, dimi endIdx, dimi isBGCol)
 ...
 bgCol = 64520
 ÄC
 ÄB selIdxRow >=0 and selIdxRow < 7 Ä
 ÄB scheduleIdx(selIdxRow, i) = 1 Ä
 bgCol = 7355
 ÄC
 bgCol = 52793
 ÄD
 ...
 Ä\(xPos, yPos, SCH_IDX_W, SCH_IDX_H, bgCol)
 xPos += 3 + SCH_IDX_GAP
 Ä
 ...
 * Description: 
 Procedure Ä decalculate the scheduled timings from the 
...
Äh deCalculation()
 ...
 Ä recordSchedule = 0 Ä MAX_DAYS-1
 ...
 selFlag=0
 cntNo=0 
 ÄB strtoint(scheduleStr$(4)) = day Ä
 ÄB strtoint(scheduleStr$(2))=1 or strtoint(scheduleStr$(2))=0 Ä
 ÄB (scheduleStr$(2))=0 Ä
 Ä chkidx=5 Ä 16 
 ÄB scheduleStr$(chkidx)="0" Ä
 cntNo++
 ÄD
 Ä
 ÄD
 
 Ä cntNo=12 Ä continue
 
 startHrs=strtoint(frmHrs$(index))
 startMin=strtoint(frmMin$(index))
 endHrs=strtoint(toHrs$(index))
 endMin=strtoint(toMin$(index))
 ÄB (startHrs+startMin+endHrs+endMin)=0 Ä
 starTime = 0
 endTime = 95
 ÄC  
 Ä startMin>0 Ä startMin-=1  
 starTime = (startHrs*60 + (startMin))/15
 endTime = (endHrs*60 + (endMin-1))/15
 ÄD  
 
 Ä scheduleTime = starTime Ä endTime
 ÄB scheduleTime < totSchedule Ä 
 ÄB recordSchedule=0 Ä
 Ä #chkschedule1=1 Ä scheduleIdx(day-1,scheduleTime)=1
 Ä #chkschedule1=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=1 Ä
 Ä #chkschedule2=1 Ä scheduleIdx(day-1,scheduleTime)=2
 Ä #chkschedule2=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=2 Ä
 Ä #chkschedule3=1 Ä scheduleIdx(day-1,scheduleTime)=3
 Ä #chkschedule3=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=3 Ä
 Ä #chkschedule4=1 Ä scheduleIdx(day-1,scheduleTime)=4
 Ä #chkschedule4=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=4 Ä
 Ä #chkschedule5=1 Ä scheduleIdx(day-1,scheduleTime)=5
 Ä #chkschedule5=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=5 Ä
 Ä #chkschedule6=1 Ä scheduleIdx(day-1,scheduleTime)=6
 Ä #chkschedule6=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 Äy recordSchedule=6 Ä
 Ä #chkschedule7=1 Ä scheduleIdx(day-1,scheduleTime)=7
 Ä #chkschedule7=0 Ä scheduleIdx(day-1,scheduleTime)=-1
 ÄD
 ÄD
 ...
 * Description: 
 * Ä store the tooltip information Ä the selected
 schedule
 * 
 * Params:
 ...
 * History: 
 ***********************************************************/
Äh storeToolTipInfo(dimi Index,dims scheduleStr$)
 Dimi tempNo
 Dims ctrlNameToHrs$
 Dims ctrlNameToMin$
 ...
 ÄB strtoint(#{ctrlNameToHrs$})<10 Ä 
 toHrs$(Index)="0"+#{ctrlNameToHrs$}
 ÄC
 toHrs$(Index)=#{ctrlNameToHrs$}
 ÄD
 
 ÄB strtoint(#{ctrlNameToMin$})<10 Ä 
 ...
 * Description: 
 * Ä clear all the selected schedules from the scheduler
 * 
 * Params:
 * Created by: Franklin Jacques.K On 2009-09-01 12:40:37
 * History: 
 ***********************************************************/
Äh ClearSchedule(dimi curVal)
 dimi row, col
 
 Ä row = 0 Ä MAX_DAYS-1
 Ä col=0 Ä totSchedule-1
 ÄB scheduleIdx(row, col) = curVal Ä
 scheduleIdx(row, col) = 0
 ÄD
 Ä
 Ä
 ...
 * Description: Ä remove all the schedules of the storage settings 
...
Äh btnRemove_Click
 Ä canReload = 0 Ä Ä
 paintFlag=0
 Ä("Do you want to remove all selected schedules",3) 
 Ä0(2)
 Ä-(2)
 ÄB Äu()=1 Ä  
 paintFlag=1
 ...
 Ä scheduleIndex=0 Ä MAX_SCHEDULE-1  
 schedule$(scheduleIndex)="0"+scheduleIndex+"007000000000000"
 Ä
 
 removeSHDLFlag=1
 
 Ä scheduleIndex=0 Ä 6  
 ÄB recordschedule$(scheduleIndex)<>"0"+scheduleIndex+"007000000000000" Ä
 httpFlag=1
 ÄE
 ÄD
 Ä
 ...
 * Description:Ä set the option button value 

 * Created by:Franklin  On 2009-07-16 12:47:57
 ***********************************************************/
Äh optrepeatschedule_Click  
 ...
 * Description: 
 * Ä check the keypress event Ä scroll & textbox validation
 * 
 * Params:
...
Äh chkUploadViaFtp_Blur
 Ä∂(3)
ÄF

Äh chkLocalStorage_Focus
 Ä #chkLocalStorage.disabled = 0 Ä Ä∂(1)
ÄF

Äh chkLocalStorage_Blur
 Ä∂(3)
ÄF

Äh optlocalStorage_Blur
 Ä∂(3)
ÄF

Äh optlocalStorage_Focus
 ÄB #optlocalStorage.disabled = 0 Ä  
 Ä∂(1)
 ÄD
ÄF

Äh optrepeatschedule_Focus
 Ä∂(1)
ÄF

Äh optrepeatschedule_Blur
 Ä∂(3)
ÄF

Äh optruntimeinfinite_Focus
 Ä∂(1)
ÄF



Äh optruntimeinfinite_Blur
 Ä∂(3)
ÄF

Äh chkUploadViaFtp_Focus
 Ä #chkUploadViaFtp.disabled = 0 Ä Ä∂(1)
ÄF

/***********************************************************

 * Description: 
 * Ä check the checked property of chkUploadViaFtp(check box ctrl)
 and set the disable property Ä ddstorageformat(drop down ctrl)

 * Created by:Franklin  On 2009-07-16 11:15:51
 ***********************************************************/
Äh chkUploadViaFtp_Click
 
 ÄB #chkUploadViaFtp.checked=0 Ä
 ...
 * Description: 
 * Ä check the checked property of chkLocalStorage(check box ctrl)
 and set the disable property Ä ddstorageformat(drop down ctrl) & 
 ...
 * History: 
 ***********************************************************/
Äh chkLocalStorage_Click
 
 ÄB #chkLocalStorage.checked=0 Ä
 #ddstorageformat1.disabled=1
 #optlocalStorage[0].disabled=1  
 ÄC
 ...
 * Description: Displays modal window Ä select schedule details
 ...
 * Description: Ä frSchedule_Cancel Ä Ä hide modal window 
 * Ä select schedule details.
 * Hide save,cancel and remove all button.
 *
 * Created by: S.Vimala On 2009-08-31 18:06:17
 * History: 
 ***********************************************************/
Äh btnFrameCancel_Click  
 Ä setValuesToDropDown(recordSchedule$) 
 Ä frSchedule_Cancel
ÄF
 ...
 * Description: Ä this Ä enable/disable drop down control based on 
 ...
 * History: 
 ***********************************************************/
Äh enableDrpCtrls(dims ctrlName$,dimi ctrlIndex) 
 dimi j
 ...
 * Description: 
 * save the schedules selected using the dropdown boxes and set Ä the 
...
Äh btnFrameOK_Click
 ...
 ÄB  #{ctrlName1$}$="00" and #{ctrlName2$}$="00" and #{ctrlName5$}$="00" and #{ctrlName4$}$="00" and diffValue$="00" and #{ctrlName$}.checked=1 Ä
 recordSchedule$(i-1) = "0"+scheduleNo+#{ctrlName$}+"0"+day+#{ctrlName2$}$+#{ctrlName4$}$+"00"+"23"+"59"+"59"  
 ÄC
 ÄB strtoint(#{ctrlName4$}$)>strtoint(#{ctrlName5$}$) Ä
...
Äh chkSchedule1_Click
 enableDrpCtrls("chkSchedule1",1)
ÄF

Äh chkSchedule2_Click
 enableDrpCtrls("chkSchedule2",2)
ÄF

Äh chkSchedule3_Click
 enableDrpCtrls("chkSchedule3",3)
ÄF

Äh chkSchedule4_Click
 enableDrpCtrls("chkSchedule4",4)
ÄF

Äh chkSchedule5_Click
 enableDrpCtrls("chkSchedule5",5)
ÄF

Äh chkSchedule6_Click
 enableDrpCtrls("chkSchedule6",6)
ÄF

Äh chkSchedule7_Click
 enableDrpCtrls("chkSchedule7",7)
ÄF

/***********************************************************

 * Description: Ä this Ä Ä assign ToHours,ToMins Ä FromHours,FromMins. 
 *
 * Params:
...
/*Ä assginDrpVal Ä Ä assign ToHours,ToMins Ä FromHours,FromMins */
Äh drpFromHrs1_Change
 assginDrpVal(1)
ÄF
...
Äh drptomin1_Change
 ÄB #drptomin1$<#drpfrommin1$ and #drptohrs1$= #drpfromhrs1$ Ä
 assginDrpVal(1)
 ÄD
ÄF


Äh drpfrommin1_change
 ÄB #drptohrs1$= #drpfromhrs1$  and #drptomin1$<#drpfrommin1$ Ä
 #drptomin1$ = #drpfrommin1$ 
 ÄD 
ÄF

Äh drpFromHrs2_Change
 assginDrpVal(2)
ÄF
...
Äh drptomin2_Change
 ÄB #drptomin2$<#drpfrommin2$ and #drptohrs2$= #drpfromhrs2$ Ä
 assginDrpVal(2)
 ÄD
ÄF


Äh drpfrommin2_change
 ÄB #drptohrs2$= #drpfromhrs2$  and #drptomin2$<#drpfrommin2$ Ä
 #drptomin2$ = #drpfrommin2$ 
 ÄD 
ÄF


Äh drpFromHrs3_Change
 assginDrpVal(3)
ÄF
...
Äh drptomin3_Change
 ÄB #drptomin3$<#drpfrommin3$ and #drptohrs3$= #drpfromhrs3$ Ä
 assginDrpVal(3)
 ÄD
ÄF


Äh drpfrommin3_change
 ÄB #drptohrs3$= #drpfromhrs3$  and #drptomin3$<#drpfrommin3$ Ä
 #drptomin3$ = #drpfrommin3$ 
 ÄD 
ÄF

Äh drpFromHrs4_Change
 assginDrpVal(4)
ÄF
...
Äh drptomin4_Change
 ÄB #drptomin4$<#drpfrommin4$ and #drptohrs4$= #drpfromhrs4$ Ä
 assginDrpVal(4)
 ÄD
ÄF


Äh drpfrommin4_change
 ÄB #drptohrs4$= #drpfromhrs4$  and #drptomin4$<#drpfrommin4$ Ä
 #drptomin4$ = #drpfrommin4$ 
 ÄD 
ÄF

Äh drpFromHrs5_Change
 assginDrpVal(5)
ÄF
...
Äh drptomin5_Change
 ÄB #drptomin5$<#drpfrommin5$ and #drptohrs5$= #drpfromhrs5$ Ä
 assginDrpVal(5)
 ÄD
ÄF


Äh drpfrommin5_change
 ÄB #drptohrs5$= #drpfromhrs5$  and #drptomin5$<#drpfrommin5$ Ä
 #drptomin5$ = #drpfrommin5$ 
 ÄD 
ÄF

Äh drpFromHrs6_Change
 assginDrpVal(6)
ÄF
...
Äh drptomin6_Change
 ÄB #drptomin6$<#drpfrommin6$ and #drptohrs6$= #drpfromhrs6$ Ä
 assginDrpVal(6)
 ÄD
ÄF


Äh drpfrommin6_change
 ÄB #drptohrs6$= #drpfromhrs6$  and #drptomin6$<#drpfrommin6$ Ä
 #drptomin6$ = #drpfrommin6$ 
 ÄD 
ÄF

Äh drpFromHrs7_Change
 assginDrpVal(7)
ÄF
...
Äh drptomin7_Change
 ÄB #drptomin7$<#drpfrommin7$ and #drptohrs7$= #drpfromhrs7$ Ä
 assginDrpVal(7)
 ÄD
ÄF

Äh drpfrommin7_change
 ÄB #drptohrs7$= #drpfromhrs7$  and #drptomin7$<#drpfrommin7$ Ä
 ...
 * Description: Ä this Ä Ä check SD card is mounted or not
 * Created by:Vimala  On 2009-11-06 16:36:20
 * History: 
 ***********************************************************/
Ä checkSDInsertValue() 
